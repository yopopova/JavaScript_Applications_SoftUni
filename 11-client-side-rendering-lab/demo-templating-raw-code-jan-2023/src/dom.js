export function dom(tagName, attributes = {}, ...content) {
    ...
    for (let propName in attributes) {
        ...
        // element.className = propName === attributes.className === user-block
    }

    for (let item of content) {
        element.append(item); // Here use 'append()' instead of 'appendChild()', because 'appendChild()' doesn't work with strings, but we have string in the last argument.
    }
