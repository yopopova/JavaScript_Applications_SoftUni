...
function fetchPost() {
  const postId = localStorage.getItem("postId"); // Like this we get the id on this page!!!
  loadPost(postId);
}
...
async function loadPost(postId) {
  try {
    ...
    const comments = await loadComments(postId); // Like this we load all comments for the current post!!!
...
async function loadComments(postId) { // Here we make request to all comments
  try {
    const res = await fetch("http://localhost:3030/jsonstore/collections/myboard/comments");
    ...
    return Object.values(comments).filter((comment) => comment.postId === postId); // This return array from elements.
    // Here we search for all comments with the current id!!!
